...
def run_slicer(stl_path: str, infill: int = 20, layer_height: float = 0.2) -> dict:
    ...
    try:
        ...
        # Wymuszamy brak środowiska graficznego
        env = os.environ.copy()
        env["DISPLAY"] = ""
        ...
        if result.returncode != 0:
            print(f"[SLICER RETURN CODE {result.returncode}]")
            print(f"[SLICER STDOUT]: {result.stdout}")
            print(f"[SLICER STDERR]: {result.stderr}")
            raise RuntimeError(f"Slicer failed: {result.stderr or result.stdout}")
